...
% network parameters
N = net.networkObject.numnodes;
E = weightedA(net.networkObject);  % transposed because of `dot` in kuramoto
...
for iter = 1:steps-1
    ...
    thetaPairwiseDiffs = theta(:, iter)*ones(1, N) - (theta(:, iter)*ones(1, N))'; 
...
function dTh = kuramoto(x, omega, lam, N, varargin)
    ...
    E_k = varargin{1};
    dTh = ones(N, 1)*omega + (ones(N, 1)*((lam / N) * dot(E_k, sin(x))))';
end
...
function res = rk4Step(dxdt, x, h, varargin)
    ...
    N_x = size(x, 1);
    f1 = dxdt(x, varargin{:});
    f2 = dxdt(x + 0.5*h*f1(:, 1)*ones(1, N_x), varargin{:});
    f3 = dxdt(x + 0.5*h*f2(:, 1)*ones(1, N_x), varargin{:});
    f4 = dxdt(x + h*f3(:, 1)*ones(1, N_x), varargin{:});        
